...
for {set minorver 1} {$minorver <= $totalnum} {incr minorver} {
	open_project /proj/equus/LOGIC_IP/LOGIC_IP.xpr
	cd /proj/equus/LOGIC_IP/LOGIC_IP.srcs/sources_1/imports/new/
	[exec rm {LUT.v}]
	[exec cp [format /proj/equus/CPTEST/automation/TEST%d.v $minorver] /proj/equus/LOGIC_IP/LOGIC_IP.srcs/sources_1/imports/new/LUT.v]
	ipx::open_ipxact_file /proj/equus/LOGIC_IP/LOGIC_IP.srcs/sources_1/imports/new/component.xml
	...
	ipx::save_core [ipx::current_core]
	update_ip_catalog -rebuild -repo_path /proj/equus/LOGIC_IP/LOGIC_IP.srcs/sources_1/imports/new/
	ipx::check_integrity -quiet -xrt [ipx::current_core]
	close_project

	open_project /proj/equus/FPGA_TOP/FPGA_TOP.xpr
	open_bd_design {/proj/equus/FPGA_TOP/FPGA_TOP.srcs/sources_1/bd/fpga_top/fpga_top.bd}
	...
	if {[get_property PROGRESS [get_runs synth_1]] != "100%"} {
		...
		write_hw_platform -fixed -include_bit -force -file [format /proj/equus/FPGA_TOP/CREATED_BITS/v%d_%d_wrapper.xsa $majorver $minorver]
	}
	close_project
}
