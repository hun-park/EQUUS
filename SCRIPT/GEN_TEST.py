...
def checkandmake(ITER, PAT):
    ...
    if os.path.exists(f"../CPTEST/{patternname}"):
...
for testnum in FDICT.keys():
    ...
    patternname = f"TEST{testnum}.pat"
    #print(os.path.exists(f"../CPTEST/{patternname}"), patternname)
    with open(f"../CPTEST/{patternname}", 'w') as TESTFILE:
